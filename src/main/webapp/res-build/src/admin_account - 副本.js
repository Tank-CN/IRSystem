...
define(function(require, exports, module) {
    ...
    var listTpl = juicer(
        [
            ...
            '    <td class=" heading">',
            ' <button type="button" class="btn btn-default btn-xs j-disable j-edit" data-toggle="modal" data-target="#modifyModal"  data-usernema="${item.username}"  data-orgs="${item.username}"  data-id="${item.id}" data-orgid="${item.orgid}"><span class="iconfont iconfont-xs">&#xe62d;</span>查看</button>',
    ...
    var Utilitiy = {
        ...
        bind: function() {
            ...
            $modifyModal.on("change", "input[name='orgs']", function(e) {
                ...
                var data_orgname = $label.attr("data-orgname");
                var data_orgid = $label.attr("data-orgid");
                $("#orgname2").val(data_orgname);
                $("#orgid2").val(data_orgid);
                if (this.checked) {
        ...
        //     $.post(createObjPath, {
        //         uid: $addRoleModal.find('input[name=id]').val(),
        //         rid: roleId
        //     }, function(data) {
